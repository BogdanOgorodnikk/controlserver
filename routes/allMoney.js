    ...
    try {
        ...
        if(ctx.user.role_id === 1 || ctx.user.role_id === 3 || ctx.user.role_id === 4) {
            ...
            const cashlessmoneys = await sequelize.query(
                ...
                WHERE (towns.manager_id = ${ctx.user.id} or towns.safemanager_id = ${ctx.user.id} or towns.securitymanager_id = ${ctx.user.id} or towns.second_security_manager_id = ${ctx.user.id} or towns.third_security_manager_id = ${ctx.user.id} or towns.fourth_security_manager_id = ${ctx.user.id} or towns.fiveth_security_manager_id = ${ctx.user.id}) and pay_cashless != 0 and DATE(orders.data) BETWEEN '${preparedDataStart}' AND '${preparedDataEnd}'
                ORDER BY orders.id`
            )
            return ctx.body = {
                cashlessmoneys: cashlessmoneys[0]
