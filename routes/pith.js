...
router.get('/api/pith/:client_id', authMiddleware, async ctx => {
    ...
    try {
        ...
        const client = await sequelize.query(
            `SELECT id, name, town_id FROM clients where id = ${client_id}`
        )
        ...
        const piths = await sequelize.query(
            `SELECT piths.id, piths.data, piths.name, piths.number, piths.price_cash, piths.price_rosdb, piths.math, piths.client_id, piths.creater, piths.data_create, users.login FROM piths 
...
router.get('/api/townpith/:town_id', authMiddleware, async ctx => {
    ...
    try {
        ...
        const clients = await sequelize.query(
            `SELECT id, name, town_id FROM clients where town_id = ${town_id}`
        )
        const piths = await sequelize.query(
