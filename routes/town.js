...
router.get('/api/town', authMiddleware, async ctx => {
    try {
        if(ctx.user.role_id < 1 || ctx.user.role_id > 6 || ctx.user.ban == 1) {
            ...
            const towns = await sequelize.query(
                `SELECT * FROM towns where manager_id = ${ctx.user.id} or safemanager_id = ${ctx.user.id} or securitymanager_id = ${ctx.user.id}`
            )
            return ctx.body = towns[0]
        }
    } catch (e) {
...
router.put('/api/town/manager/:id', authMiddleware, async ctx => {
    const {manager_id, safemanager_id, securitymanager_id} = ctx.request.body
    try {
        if(ctx.user.role_id != 1 || ctx.user.role_id == 1 && ctx.user.ban == 1) {
            return ctx.status = 400
        }
        const town = await Town.update(
            {manager_id: manager_id, safemanager_id: safemanager_id, securitymanager_id: securitymanager_id},
            {where: {id: ctx.params.id}}
        )
        ctx.body = town
    } catch (e) {
