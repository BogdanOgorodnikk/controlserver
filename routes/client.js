...
router.get('/api/client/:town_id', authMiddleware, async ctx => {
    ...
    try {
        ...
        } else if(ctx.user.role_id == 5 && townManagers[0][0].manager_id != ctx.user.id && townManagers[0][0].safemanager_id != ctx.user.id && townManagers[0][0].securitymanager_id != ctx.user.id) {
            return ctx.status = 400
        }
