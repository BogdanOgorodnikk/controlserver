        ...
        if(ctx.user.role_id == 1 && ctx.user.ban == 0) {
            const order = await sequelize.query(
                `SELECT orders.id, orders.isSelfCar, orders.order_number, orders.note, orders.comment, orders.car_number,
                    ...
                    where client_id = ${client_id} AND DATE(orders.data) BETWEEN '${preparedDataStart}' AND '${preparedDataEnd}'
                    ORDER BY orders.id`
            )
            ...
            let prepareOrders = await sequelize.query(
                `SELECT prepare_orders.id, prepare_orders.comment, prepare_orders.price_cash, prepare_orders.note,
                 ...
                 WHERE prepare_orders.order_number = "" and prepare_orders.client_id = ${client_id} and DATE(prepare_orders.data) BETWEEN '${preparedDataStart}' AND '${preparedDataEnd}'
            ...
            const order = await sequelize.query(
                ...
                FROM orders 
                where client_id = ${client_id} and firm != "" and orders.pay_cashless = 0 and DATE(orders.data) BETWEEN '${preparedDataStart}' AND '${preparedDataEnd}'
                ORDER BY id`
            )
            ...
            const order = await sequelize.query(
                `SELECT id, isSelfCar, order_number, orders.note, orders.comment, car_number, original_data_update,
