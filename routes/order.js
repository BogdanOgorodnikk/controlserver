...
router.get('/api/orders/:client_id', authMiddleware, async ctx => {
    ...
    try {
        ...
        if(ctx.user.role_id == 1 && ctx.user.ban == 0) {
            ...
            const order = await sequelize.query(
                `SELECT order_number, car_number, firm, data, product_name, opt_price, count, delivery_cash, delivery_cashless, region FROM orders where client_id = ${client_id} and firm != ""`
            )
            return ctx.body = {
                order: order[0]
            }
            ...
            const order = await sequelize.query(
                `SELECT id, order_number, car_number, firm, data, product_name, opt_price, price_cash, count, sumseller, delivery_cash, delivery_cashless, pay_cash, pay_cashless, region FROM orders where client_id = ${client_id}`
            )
            return ctx.body = {
                order: order[0]
            }
            ...
            const order = await sequelize.query(
                `SELECT id, order_number, car_number, firm, data, product_name, opt_price, price_cash, price_cashless, count, delivery_cash, delivery_cashless, pay_cashless, region FROM orders where client_id = ${client_id}`
            )
            return ctx.body = {
                order: order[0]
            }
            ...
            if(ctx.user.role_id == 5 && town[0][0].manager_id != ctx.user.id && town[0][0].safemanager_id != ctx.user.id && town[0][0].securitymanager_id != ctx.user.id) {
                return ctx.status = 400
            }
            const order = await sequelize.query(
                `SELECT id, firm, data, product_name, price_cash, price_cashless, count, sumseller, delivery_cash, delivery_cashless, general_sum, pay_cash, pay_cashless, region FROM orders where client_id = ${client_id}`
            )
            return ctx.body = {
                order: order[0],
                client: client[0],
...
router.get('/api/suminfo/:client_id', authMiddleware, async ctx => {
    ...
    try {
        ...
        const sumPith = await sequelize.query(
            `SELECT sum(price_cash*(number * 1.4)) as sumPith FROM piths where client_id = ${client_id} and math = 1`
        )
        return ctx.body = {
            sumMas: sumMas[0],
            sumPith: sumPith[0]
...
router.post('/api/orders/:client_id', authMiddleware, async ctx => {
    const {order_number, car_number, firm, region, data, product_name, opt_price, count, delivery_cash, delivery_cashless} = ctx.request.body
    try {
...
router.put('/api/editorder/:id', authMiddleware, async ctx => {
    const {order_number, car_number, firm, region, data, product_name, count, delivery_cash, delivery_cashless, price_cash, opt_price, price_cashless, client_id} = ctx.request.body
    try {
        ...
        const order = await Order.update(
            {
                ...
                delta_cash: ((price_cash - 0) * count) - delivery_cash,
                delta_cashless: ((0 - opt_price) * count) - delivery_cashless,
